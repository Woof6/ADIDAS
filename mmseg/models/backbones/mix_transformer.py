    ...
    def __init__(self,
        ...
        if time_dim:
            self.to_scale_shift = nn.Sequential(
                nn.SiLU(),
                nn.Linear(time_dim, hidden_features * 2),
                Rearrange('b d -> b 1 d')
            )
...
from .myddpm import Unet,default,extract,sigmoid_beta_schedule
...
class MixVisionTransformer(BaseModule):
    def __init__(self,
        ...
        if time_dim:
            ...
            # sinu_pos_emb = LearnedSinusoidalPosEmb(16)
            # fourier_dim = 16+ 1
            sinu_pos_emb = SinusoidalPosEmb(16)
            fourier_dim = 16
            self.time_mlp = nn.Sequential(
                sinu_pos_emb,
        ...
        self.block1 = nn.ModuleList([
            Block(
                ...
                time_dim= time_dim ) for i in range(depths[0])
        ])
        ...
        self.block2 = nn.ModuleList([
            Block(
                ...
                time_dim= time_dim ) for i in range(depths[1])
        ])
        ...
        self.block3 = nn.ModuleList([
            Block(
                ...
                time_dim= time_dim ) for i in range(depths[2])
        ])
        ...
        self.block4 = nn.ModuleList([
            Block(
                ...
                time_dim= time_dim ) for i in range(depths[3])
        ])
    ...
    def forward(self, x):
        if isinstance(x,list):
            x = self.forward_features(x[0],x[1])
        else:
...
@BACKBONES.register_module()
class mit_b5_denoise(MixVisionTransformer):

    def __init__(self, **kwargs):
        super(mit_b5_denoise, self).__init__(
            patch_size=4,
            embed_dims=[64, 128, 320, 512],
            num_heads=[1, 2, 5, 8],
            mlp_ratios=[4, 4, 4, 4],
